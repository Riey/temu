...
[[block]]
struct CellInfos {
    infos: array<CellInfo>;
};

[[group(0), binding(0)]] var<uniform> window_size: WindowSizeUniform;
[[group(0), binding(1)]] var<storage, read> cell_infos: CellInfos;
...
fn calculate_cell_coord(cell_index: u32) -> vec2<f32> {
    let row: u32 = cell_index / window_size.column;
    let column: u32 = cell_index % window_size.column;
    return vec2<f32>(f32(column), f32(row));
}

fn calculate_cell_rect(coord: vec2<f32>) -> Rect {
    let begin = coord * window_size.cell_size;
...
fn cell_vs(
    ...
    let info = cell_infos.infos[model.cell_index];
    let coord = calculate_cell_coord(model.cell_index);
    let rect = calculate_cell_rect(coord);
